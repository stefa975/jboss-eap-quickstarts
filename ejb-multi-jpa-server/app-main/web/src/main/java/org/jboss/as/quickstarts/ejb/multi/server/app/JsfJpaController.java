 ...
 */
@Model
public class JsfJpaController {
    private static final Logger LOOGER = Logger.getLogger(JsfJpaController.class);
     ...
     */
    @Resource(lookup = "ejb:ejb-multi-jpa-server-app-one/ejb//AppJpaOneBean!org.jboss.as.quickstarts.ejb.multi.server.app.AppJpaOne")
    AppJpaOne oneApp;
     ...
     */
    @Resource(mappedName = "ejb:ejb-multi-jpa-server-app-two/ejb//AppJpaTwoBean!org.jboss.as.quickstarts.ejb.multi.server.app.AppJpaTwo")
    AppJpaTwo twoApp;
    ...
    public void callEJBMainJpaLocal() {
        LOOGER.info("Try to invoke the local MainApp to log the given text and get the invocation results. Proxy=" + mainApp);
        this.invocation.setResult(mainApp.invokeJpaAll(this.invocation.getText()));
    }
