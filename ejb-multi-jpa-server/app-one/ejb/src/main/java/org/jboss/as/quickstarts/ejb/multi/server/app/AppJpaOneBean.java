...
import org.jboss.as.quickstarts.ejb.multi.server.app.model.Customer;
 ...
 */
@Stateless
public class AppJpaOneBean implements AppJpaOne {
    private static final Logger LOGGER = Logger.getLogger(AppJpaOneBean.class);
    ...
    public String invokeJpa(String name) {
        Customer c1 = new Customer();
        c1.setName(name);
        entityManager.persist(c1);
        LOGGER.info("[ persist ] " + name);

        Principal caller = context.getCallerPrincipal();
        LOGGER.info("[" + caller.getName() + "] " + name);
        return "app1[" + caller.getName() + "]@" + getJBossNodeName();
